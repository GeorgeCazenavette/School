...
int main(void)
{
	...
	// Initialize the Standard I/O UART
	...
	unsigned char sum = 0;
	unsigned char ch;
	unsigned int leftOfDec = 0;
	unsigned int rightOfDec = 0;
	unsigned char array[CHAR_COUNT];
	while (true)
	{
		
		counter = 0;
		printf("Enter %d numbers to be averaged\n", (CHAR_COUNT - counter));
		
		while(counter != CHAR_COUNT)
		{
			ch = getchar();
			if (ch >= 0b00110000 && ch <= 0b00111001)
			{
				putchar(ch);
				puts("\n");
				array[counter] = ch - '0';
		...
		leftOfDec = sum / CHAR_COUNT;
		rightOfDec = (sum % CHAR_COUNT) * 1000 / CHAR_COUNT;
		printf("Sum = %d \nCount = %d\nAverage = %d.%d\n\n", sum, CHAR_COUNT, leftOfDec, rightOfDec);
